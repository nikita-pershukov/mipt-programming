...
void insertsort(void* arr, int n, int size, int (*compare)(const void
*a, const void* b))
{
    if (n == 1)
        return;
...
int main()
{
	...
	while (1)
	{
		...
		if (strcmp(com, "insert\0") == 0)
		{
			...
			int value = 0;
			scanf("%d", &value);
			heap_insert(a, &n, sizeof(int), compare_int, &value);
		}
		if (strcmp(com, "increase\0") == 0)
		{
			printf("Insert ind & value\n");
			int ind = 0;
			int value = 0;
			scanf("%d", &ind);
			scanf("%d", &value);
			heap_increase_key(a, ind, sizeof(int), compare_int, &value);
		}
		if (strcmp(com, "max\0") == 0)
		{
			int* max = heap_extract_max(a, &n, sizeof(int), compare_int);
			if (max)
				printf("max %d\n", *max);
		}
		if (strcmp(com, "sort\0") == 0)
			heapsort_insert(a, n, sizeof(int), compare_int, 16);
		...
		printf("\n---\n");
		if (strcmp(com, "sort\0") == 0)
			buildheap(a, n, sizeof(int), compare_int);
	}
